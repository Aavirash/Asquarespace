...
const space2CollectionsToggle = document.getElementById('space2-collections-toggle');
const space2GridAdd = document.getElementById('space2-grid-add');
const space2DiscoverTab = document.getElementById('space2-discover-tab');
const space2DiscoverCorner = document.getElementById('space2-discover-corner');
...
function renderSpace2Collections(){
    ...
    space2State.collections.forEach(col=>{
        ...
        row.className=`space2-col-item${space2ActiveCollection===col.id?' active':''}`;
        row.innerHTML=`<span>${col.name}</span><span>${count}</span>`;
...
function sendDiscoverItem(item,target){
    ...
    const activeCollectionIds=space2ActiveCollection!=='all'?[space2ActiveCollection]:[];
...
function updateControlCornerState(){
    ...
    if(workflowDock){
        workflowDock.classList.toggle('hidden',!(IS_MAC_APP && currentSpace==='space1'));
    }
    if(space2DiscoverCorner){
        space2DiscoverCorner.classList.toggle('hidden',!(currentSpace==='space2' && space2View==='grid'));
    }
}

function initSpace2SidebarSizing(){
    if(!space2Sash||!space2Panel) return;
    const saved=parseInt(localStorage.getItem('asq.space2.sidebar.width')||'238',10);
    if(Number.isFinite(saved)) setSpace2SidebarWidth(saved,{persist:false});

    let dragging=false;
    let moved=false;
    const onPointerMove=(e)=>{
        if(!dragging||!space2Panel) return;
        moved=true;
        const rect=space2Panel.getBoundingClientRect();
        const raw=e.clientX-rect.left;
        const width=Math.max(0,Math.min(380,Math.round(raw)));
        if(width<=170){
            if(space2Panel) space2Panel.style.setProperty('--space2-sidebar-width','170px');
            setSpace2CollectionsOpen(false,{skipPersist:true});
            return;
        }
        setSpace2CollectionsOpen(true,{skipPersist:true});
        setSpace2SidebarWidth(width,{persist:false});
    };
    const onPointerUp=()=>{
        if(!dragging) return;
        dragging=false;
        if(space2Panel) space2Panel.classList.remove('space2-resizing');
        if(!moved){
        ...
        const visualWidth=space2Panel?.classList.contains('sidebar-collapsed') ? 0 : space2SidebarWidth;
        const shouldClose=visualWidth<190;
        if(shouldClose) setSpace2CollectionsOpen(false);
        else {
            setSpace2CollectionsOpen(true);
            setSpace2SidebarWidth(space2SidebarWidth,{persist:true});
        }
        window.removeEventListener('pointermove',onPointerMove);
        window.removeEventListener('pointerup',onPointerUp);
    };

    space2Sash.addEventListener('pointerdown',(e)=>{
        e.preventDefault();
        dragging=true;
        moved=false;
        if(space2Panel) space2Panel.classList.add('space2-resizing');
        if(!space2CollectionsOpen) setSpace2CollectionsOpen(true,{skipPersist:true});
...
function setSpace(space){
    ...
    if(currentSpace==='space2'){
        ...
        if(space2Search) space2Search.focus({preventScroll:true});
    }else{
...
function showSpace2View(view) {
    ...
    if(space2CollectionsToggle) space2CollectionsToggle.classList.toggle('active', isGrid);
    if(space2DiscoverTab) space2DiscoverTab.classList.toggle('active', !isGrid);
...
if(space2CollectionsToggle) space2CollectionsToggle.addEventListener('click', () => showSpace2View('grid'));
if(space2DiscoverTab) space2DiscoverTab.addEventListener('click', () => showSpace2View('discover'));
if(space2DiscoverCorner) space2DiscoverCorner.addEventListener('click', () => showSpace2View('discover'));
...
function renderDiscoverCard(item) {
    ...
    card.innerHTML = `
        ...
        <div class="discover-card-actions">
            <button class="discover-save-btn" data-target="grid" title="Send to main grid">Grid</button>
            <button class="discover-save-btn" data-target="collection" title="Send to a collection">Collection</button>
        </div>
    `;
    ...
    card.querySelectorAll('.discover-save-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            btn.textContent = state === 'collections' ? 'Assign' : 'Sent';
            setTimeout(()=>{
                delete btn.dataset.state;
                btn.textContent = btn.dataset.target === 'grid' ? 'Grid' : 'Collection';
            },1400);
        });
    });
